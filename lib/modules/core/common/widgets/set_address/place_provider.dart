...
/// [PlaceProvider] This class is used to Place Provider
class PlaceProvider extends ChangeNotifier {
  ...
  /// [updateCurrentLocation] This method is used to update Current Location
  Future<void> updateCurrentLocation(bool forceAndroidLocationManager) async {
    try {
      ...
      } else if (await Permission.location.request().isLimited) {
        currentPosition = await Geolocator.getCurrentPosition(
            desiredAccuracy: desiredAccuracy ?? LocationAccuracy.best);
      } else if (Platform.isIOS &&
          await Permission.location.request().isRestricted) {
        currentPosition = await Geolocator.getCurrentPosition(
            desiredAccuracy: desiredAccuracy ?? LocationAccuracy.best);
      } else {
  ...
  SearchingState _placeSearchingState = SearchingState.ddle;
  ...
  /// [setMapType] This method is used to set Map Type
  setMapType(MapType mapType, {bool notify = false}) {
    _mapType = mapType;
    if (notify) notifyListeners();
  }

  /// [switchMapType] This method is used to switch Map Type
  switchMapType() {
    _mapType = MapType.values[(_mapType.index + 1) % MapType.values.length];
    if (_mapType == MapType.none) _mapType = MapType.none;
