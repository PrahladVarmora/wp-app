...
import '../bloc/resend_receipt_bloc.dart';

///[DialogResendReceipt] this class is used as Dialog Send And RequestMoney
class DialogResendReceipt extends StatefulWidget {
  final JobData mJobData;

  ///this variable is used for Description og dialog box
  final Function buttonOnTap;

  const DialogResendReceipt(
      {Key? key, required this.buttonOnTap, required this.mJobData})
      : super(key: key);

  @override
  State<DialogResendReceipt> createState() => _DialogSendAndRequestMoneyState();
}

class _DialogSendAndRequestMoneyState extends State<DialogResendReceipt> {
      ...
      return CustomButton(
        ...
        onPress: () {
          if (mSelectedCountry.value == '+') {
            ...
            BlocProvider.of<ResendReceiptBloc>(context)
                .add(ResendReceipt(url: AppUrls.apiResendReceipt, body: {
                  ...
                  Stack(
                    children: [
                      Container(
                        ...
                        child: Text(
                          APPStrings.textResendReceipt.translate(),
                          style: getTextStyleFontWeight(
                              Theme.of(context).primaryTextTheme.labelSmall!,
                              Dimens.textSize18,
                              FontWeight.w600),
